...
export default function HomeSesc({ navigation }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!isMounted) return;
      if (error) {
        console.error("Erro ao carregar produtos (SESC):", error);
...
function belongsToCategory(category, filter) {
  const normalize = (s) => {
    ...
    v = v
      ...
      .replace(/[úùû]/g, "u")
      .replace(/ç/g, "c");
    if (v.endsWith("s")) v = v.slice(0, -1); // singulariza simples
    return v;
  };
